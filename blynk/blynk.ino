...
BLYNK_WRITE(V3) {
  ...
  if (pinValue == 1) {
    ...
    int pinValue = 0;       // Defina o status V3 como 0 para sair, a menos que o botão ainda esteja pressionado (conforme abaixo)
    Blynk.syncVirtual(V3);  // ...Então força a verificação da função BLYNK_WRITE(V3) do status do botão para determinar se está repetindo ou pronto.
  }
}
...
BLYNK_WRITE(V4) {  // Stobe
  ...
  if (pinValue == 1) {
    ...
    for (int i = 1; i <= 4; i++) {
      ...
      relays.SetRelay(i, SERIAL_RELAY_ON, 4);
    }
    delay(PauseTime);
    ...
    for (int i = 1; i <= 4; i++) {
      ...
      relays.SetRelay(i, SERIAL_RELAY_OFF, 4);
    }
    delay(PauseTime);
